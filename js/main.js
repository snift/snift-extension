...
function sendXHR(tablink) {
    ...
    if (xhr.readyState == XMLHttpRequest.DONE) {
        processScores(xhr.responseText);
    }
}
url = url + tablink;
...
function processFavicon(tablink){
  document.getElementById("favicon").src = getURL(tablink)+"/favicon.ico";
  document.getElementById("domain").innerHTML = getURL(tablink);
...
function processScores(responseText){
    responseObj = JSON.parse(responseText);
    console.log(responseObj.scores.score)
    document.getElementById("chart-gauge").innerHTML = gauge(responseObj.scores.score); 
}
